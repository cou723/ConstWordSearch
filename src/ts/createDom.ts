...
//div要素の作成
export function createDiv(count: number) {
  $("#searches").append(`<div id="search${count}"></div>`);
  $(`#search${count}`).addClass("row p-0");
}

export function createForm(count: number) {
  $(`#search${count}`).append(
...
export function createSearchP(headerText: string, count: number) {
...
export function createSearchInput(count: number) {
...
export function createSearchDeleteButton(count: number) {
  $(`#searchForm${count}`).append(
